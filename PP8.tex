...
void loop() { 
	...
	Serial.print(p);
	Serial.print(" ");
	Serial.println(t1-t0); //stampa il tempo in microsecondi
	while (p > MAX) { }; //il programma si arresta quando il numero supera quello massimo
}
\end{lstlisting}
...
%\begin{Figure}
%	\begin{center}
%	\includegraphics[width=\linewidth]{quadra1.png}
...
%\begin{Figure}
%	\begin{center}
%	\includegraphics[width=\linewidth]{quadra2.png}
...
\begin{lstlisting}[style=CStyle, caption={Codice per la scrittura PWM, modulazione triangolare}, label=lst:sinusoidale]
int ledPin = 9;
int val;
float x;
float dx = 0.01;
...
\begin{lstlisting}[style=CStyle, caption={Codice per la scrittura PWM}, label=lst:calibrazioneADC]
int analogPin = 3;
int RdVal = 0;
float Vx = 0.0;
void setup() {
...
Nel grafico di Figura~\ref{fig:calibrazioneADC} sono riportati i punti sperimentali e la retta $y = mx+q$ che meglio li interpola, di coefficiente angolare $m = \SI{111}{V^{-1}}$ e intercetta $q = \SI{111}{}$, compatibili con i valori previsti $m^{\text{(atteso)}} = 1023/\SI{5.0}{V}$ e $q^{\text{(atteso)}} = 0$. Il chi quadro è, inoltre, $\chi^2 = 111$, che si confronta con il numero di gradi di libertà ($111$).
